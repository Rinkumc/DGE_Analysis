Data=read.csv("C:/Users/HP/OneDrive/Documents/raw_counts_mouseKO.csv",
              row.names = 1)
Data = log2(Data+ 1)
...
colnames(mat)=c('p_values','meanKO','meanwt','log2fc')
for (i in 1:nrow(Data)) 
  ...
  mat[i,c('meanKO','meanwt')]=T$estimate
  mat[i,'log2fc']=log2(mat[i,'meanKO']/mat[i,'meanwt'])
}
x=mat[,'log2fc']
...
head(mdf[order(mdf$meanKO) & mdf$diffexpressed == 'DOWN', ])
mdf$delabel = ifelse(mdf$p_values %in% head(mdf[order(mdf$meanKO), "pvalue"], 30), mdf$p_values, NA)
